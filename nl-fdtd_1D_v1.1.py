...
# アニメーションの設定
frame_interval, dpi, file_name = int(waveL*dx/C/dt), 100, "nl-fdtd_1D_v1.1"
...
for n in tqdm(range(M - 1)):
    ...
    #ダッシュポット想定
    Ux2[0:nx] = Ux1[0:nx] - dt * ((P1[0:nx] - cp.roll(P1[0:nx], 1)) / dx + k*Ux1[0:nx]) /(Ro + (P1[0:nx] + cp.roll(P1[0:nx], 1)) / (2 * C**2)) - dt * Ux1[0:nx] * (Ux1[1:nx+1] - cp.roll(Ux1[0:nx], 1)) / (2 * dx)
