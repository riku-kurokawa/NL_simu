...
dpi, file_name = 100, "nl-fdm_1D_v1.1"
...
p[1,50:(50 + waveL)] = cp.roll(p[0,50:(50 + waveL)], int(C*dt/dx))
u[1,50:(50 + waveL)] = cp.roll(u[0,50:(50 + waveL)], int(C*dt/dx))



for n in tqdm(range(1, M-1)):
    #p[n+1,1:nx] = p[n,1:nx] - a* ((p[n,1:nx] + d) * (u[n,2:nx + 1]-u[n,0:nx - 1]) + u[n,1:nx]*(p[n,2:nx + 1]-p[n,0:nx - 1]))
    #u[n+1,1:nx] = u[n-1,1:nx] - a* (u[n,1:nx]*(u[n,2:nx + 1]-u[n,0:nx - 1]) + (p[n,2:nx + 1]-p[n,0:nx - 1]) / (p[n,1:nx] / b + Ro))
    #p[n+1,1:nx] = p[n-1,1:nx] - a* ((d + p[n,2:nx+1])*(u[n+1,2:nx + 1]) - (d + p[n+1,0:nx-1] )*(u[n+1,0:nx - 1]))
    u[n+1,1:nx] = u[n-1,1:nx] - a* (p[n,2:nx + 1]-p[n,0:nx - 1]) / Ro
    p[n+1,1:nx] = p[n-1,1:nx] - a* Ro *b*(u[n+1,2:nx + 1] - u[n+1,0:nx - 1])
